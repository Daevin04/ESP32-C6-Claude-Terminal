...
/*Use the standard `memcpy` and `memset` instead of LVGL's own functions. (Might be more efficient)*/
 ...
 *LV_DITHER_GRADIENT implies LV_GRADIENT_MAX_STOPS = 256 (so never stops are supported)
 *but the memory usage is also higher*/
 ...
 *should also appear on LVGL binding API such as Micropython or Littlevgl-rs.*/
