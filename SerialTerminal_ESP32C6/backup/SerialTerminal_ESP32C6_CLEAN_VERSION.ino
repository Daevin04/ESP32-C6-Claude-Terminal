...
{
  // No timestamp - clean messages only
  ...
  // No auto-scroll - all scrolling is manual
}
...
void handle_hardware_buttons()
{
  ...
  {
    Serial.println("BUTTON_UP_PIN pressed: scrolling UP");
    // Manual scrolling only
    // Use scroll_by with positive value to scroll up
    lv_obj_scroll_by(terminal_area, 0, 30, LV_ANIM_ON);
    last_button_time = current_time;
  }

  // BUTTON_DOWN_PIN (GPIO 18) - scroll down in terminal
  if (down_pressed && last_down_state != down_pressed)
  {
    Serial.println("BUTTON_DOWN_PIN pressed: scrolling DOWN");
    // Manual scrolling only
    // Use scroll_by with positive value to scroll up
  ...
  {
    Serial.println("BUTTON_SEND_PIN pressed: scrolling DOWN");
    // Scroll down incrementally like DOWN button
    lv_obj_scroll_by(terminal_area, 0, -60, LV_ANIM_ON);
    last_button_time = current_time;
  }
...
void setup()
{
  ...
  Serial.println("ESP32-C6 Touch Terminal Ready!");
  Serial.println("Touch Navigation: Top half (Y<145) = UP, Bottom half = DOWN");
  Serial.println("Secondary: Physical UP/DOWN buttons");
  Serial.println("Commands: Send any text to display it");
  Serial.println("Special commands: STOP_TEST, START_TEST, STOP_SCROLL_TEST, START_SCROLL_TEST");
